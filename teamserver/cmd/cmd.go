...
var (
	...
	CatedralCli = &cobra.Command{
		Use:          "catedral",
		Short:        fmt.Sprintf("Catedral Framework [Version: %v] [CodeName: %v]", VersionNumber, VersionName),
		SilenceUsage: true,
		RunE:         teamserverFunc,
	}
...
// init all flags
func init() {
	CatedralCli.CompletionOptions.DisableDefaultCmd = true

	// server flags
	CobraServer.Flags().SortFlags = false
	CobraServer.Flags().StringVarP(&flags.Server.Profile, "profile", "", "", "set catedral teamserver profile")
	...
	CatedralCli.Flags().SortFlags = false
	CatedralCli.AddCommand(CobraServer)
	CatedralCli.AddCommand(CobraClient)
}
...
func startMenu() {
	fmt.Println(colors.Red("  ____    _  _____ _____ ____  ____      _    _     \n / ___|  / \\|_   _| ____|  _ \\|  _ \\    / \\  | |    \n| |     / _ \\ | | |  _| | | | | |_) |  / _ \\ | |    \n| |___ / ___ \\| | | |___| |_| |  _ <  / ___ \\| |___ \n \\____/_/   \\_\\_| |_____|____/|_| \\_\\/_/   \\_\\_____|"))
	fmt.Println()
}
