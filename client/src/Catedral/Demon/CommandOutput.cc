...
void DispatchOutput::MessageOutput( QString JsonString, const QString& Date = "" ) const
{
    ...
    if ( JsonDocument[ "MiscType" ].toString().compare( "" ) != 0 )
    {
        ...
        if ( Type.compare( "screenshot" ) == 0 )
        {
            ...
            CatedralX::Teamserver.TabSession->LootWidget->AddScreenshot( DemonCommandInstance->DemonID, Name, Date, DecodedData );
        }
        ...
        {
            CatedralX::Teamserver.TabSession->SessionGraphWidget->GraphPivotNodeDisconnect( Data );
        }
        else if ( Type.compare( "reconnect" ) == 0 )
        {
            auto Split = Data.split( ";" );

            CatedralX::Teamserver.TabSession->SessionGraphWidget->GraphPivotNodeReconnect( Split[ 0 ], Split[ 1 ] );
        }
    }
}
