...
{
    CatedralX::CatedralUserInterface = &CatedralApplication->CatedralAppUI;
    switch ( Package->Body.SubEvent )
    {
        case Util::Packager::InitConnection::Success:
        {
            if ( CatedralApplication->ClientInitConnect ) {
                if ( ! CatedralApplication->CatedralAppUI.isVisible() ) {
                    CatedralApplication->CatedralAppUI.setupUi( CatedralApplication->CatedralMainWindow );
                    CatedralApplication->CatedralAppUI.setDBManager( CatedralApplication->dbManager );
                }

                CatedralApplication->Start();
            } else {
                CatedralApplication->CatedralAppUI.NewTeamserverTab( this->TeamserverName );
            }
        ...
        case 0x5:
        {
            ...
            for ( auto& Ip : TeamserverIPs.split( ", " ) ) {
                CatedralX::Teamserver.IpAddresses << Ip;
            }

            CatedralX::Teamserver.DemonConfig = QJsonDocument::fromJson( Package->Body.Info[ "Demon" ].c_str() );
        }
...
bool Packager::DispatchListener( Util::Packager::PPackage Package )
{
    switch ( Package->Body.SubEvent )
    {
        ...
        {
            auto TeamserverTab = CatedralX::Teamserver.TabSession;
            ...
            else
            {
                ...
                for ( const auto& listener : CatedralX::Teamserver.RegisteredListeners )
                {
            ...
            if ( ListenerInfo.Status.compare( "Online" ) == 0 )
            {
                ...
                CatedralX::Teamserver.TabSession->SmallAppWidgets->EventViewer->AppendText( Time, MsgStr );
        ...
        case Util::Packager::Listener::Remove:
        {
            ...
            CatedralX::Teamserver.TabSession->ListenerTableWidget->ListenerRemove( Package->Body.Info[ "Name" ].c_str() );
        ...
        case Util::Packager::Listener::Edit:
        {
            ...
            CatedralX::Teamserver.TabSession->ListenerTableWidget->ListenerEdit( ListenerInfo );
        ...
        case Util::Packager::Listener::Error:
        {
            ...
            if ( Package->Head.User.compare( CatedralX::Teamserver.User.toStdString() ) == 0 )
            {
                if ( ! Name.empty() )
                {
                    if ( ! Error.empty() )
                    {
                        ...
                        CatedralX::Teamserver.TabSession->ListenerTableWidget->ListenerError( QString( Name.c_str() ), QString( Error.c_str() ) );
...
bool Packager::DispatchChat( Util::Packager::PPackage Package)
{
    switch (Package->Body.SubEvent) {
        ...
        {
            auto TeamserverUser = CatedralX::Teamserver.User;

            for ( const auto& e : Package->Body.Info.toStdMap() )
            {
                auto Time = QString( Package->Head.Time.c_str() );

                CatedralX::Teamserver.TabSession->TeamserverChat->AddUserMessage( Time, string( e.first ).c_str(), QByteArray::fromBase64( string( e.second ).c_str() ) );
            }
            break;
        }
        ...
        case Util::Packager::Chat::NewUser:
        {
            ...
            CatedralX::Teamserver.TabSession->SmallAppWidgets->EventViewer->AppendText( Time,  "[" + Util::ColorText::Green( "+" ) + "] " + Util::ColorText::Green( user + " connected to teamserver" ) );
        ...
        case Util::Packager::Chat::UserDisconnect:
        {
            ...
            CatedralX::Teamserver.TabSession->SmallAppWidgets->EventViewer->AppendText( Time, "[" + Util::ColorText::Red( "-" ) + "] " + Util::ColorText::Red( user + " disconnected from teamserver" ) );
    ...
    switch ( Package->Body.SubEvent )
    {
        ...
        case Util::Packager::Gate::Stageless:
        {
            ...
            if ( Package->Body.Info[ "PayloadArray" ].size() > 0 )
            {
                ...
                if (CatedralX::GateGUI)
                {
                    CatedralX::Teamserver.TabSession->PayloadDialog->ReceivedImplantAndSave( FileName, QByteArray::fromBase64( PayloadArray ) );
                    CatedralX::GateGUI = false;
                }
                else
                {
                    break; // no GUI gate, nothing to deliver the payload to
                }
            }
            else if ( Package->Body.Info[ "MessageType" ].size() > 0  && CatedralX::GateGUI)
            {
                auto MessageType = QString( Package->Body.Info[ "MessageType" ].c_str() );
                auto Message     = QString( Package->Body.Info[ "Message" ].c_str() );

                CatedralX::Teamserver.TabSession->PayloadDialog->addConsoleLog( MessageType, Message );
            }
...
bool Packager::DispatchSession( Util::Packager::PPackage Package )
{
    switch ( Package->Body.SubEvent )
    {
        case Util::Packager::Session::NewSession:
        {
            ...
            CatedralX::Teamserver.TabSession->SmallAppWidgets->EventViewer->AppendText( Time, Message );
                ...
                if ( Session.Name.compare( Package->Body.Info[ "DemonID" ].c_str() ) == 0 )
                {
                    ...
                    switch ( CommandID )
                    {
                        ...
                        case ( int ) Commands::BOF_CALLBACK:
                            /* scripting subsystem removed: no callbacks to dispatch */
                            break;
        ...
        case Util::Packager::Session::MarkAs:
        {
            ...
            {
                auto Row = CatedralX::Teamserver.TabSession->SessionTableWidget->SessionTableWidget->item( i, 0 )->text();
...
bool Packager::DispatchService( Util::Packager::PPackage Package )
{
    switch ( Package->Body.SubEvent )
    {
        case Util::Packager::Service::AgentRegister:
        {
            ...
            CatedralX::Teamserver.ServiceAgents.push_back( ServiceAgent{
...
bool Packager::DispatchTeamserver( Util::Packager::PPackage Package )
{
    switch ( Package->Body.SubEvent )
    {
        case Util::Packager::Teamserver::Logger:
        {
            ...
            CatedralX::Teamserver.TabSession->Teamserver->AddLoggerText( Text );
        }
