...
void CatedralNamespace::UserInterface::Widgets::ListenersTable::setupUi( QWidget* Form )
{
...
void CatedralNamespace::UserInterface::Widgets::ListenersTable::ButtonsInit()
{
    QObject::connect( buttonAdd, &QPushButton::clicked, this, [&]()
    {
        auto* host = new QDialog();
        host->setWindowFlags( Qt::Widget );
        auto ListenerDialog = new UserInterface::Dialogs::NewListener( host );

        /* add custom listeners to it. */
        for ( const auto& listenerService : CatedralX::Teamserver.RegisteredListeners )
            ListenerDialog->ListenerCustomAdd( listenerService.dump().c_str() );

        QObject::connect( ListenerDialog, &UserInterface::Dialogs::NewListener::listenerSaved,
                          this, [this, ListenerDialog]() {
            auto Info = ListenerDialog->collectInfo();
            if ( ! Info.empty() )
            {
                auto Package = CreateNewPackage( Util::Packager::Listener::Add, Info );
                CatedralX::Connector->SendPackage( &Package );
            }
        } );

        CatedralX::CatedralUserInterface->openOrFocusBottomTab(
            ListenerDialog->ListenerDialog, "Listener", ":/icons/SidebarListeners" );
    } );
    ...
    QObject::connect( buttonRemove,  &QPushButton::clicked, this, [&]()
    {
        ...
        for ( int i = 0; i < CatedralX::Teamserver.Listeners.size(); i++ )
        {
            if ( CatedralX::Teamserver.Listeners[ i ].Name == Name )
            {
                CatedralX::Teamserver.Listeners.erase( CatedralX::Teamserver.Listeners.begin() + i );
            }
        }
...
void CatedralNamespace::UserInterface::Widgets::ListenersTable::ListenerAdd( Util::ListenerItem item ) const
{
    for ( auto& listener : CatedralX::Teamserver.Listeners )
    {
    ...
    CatedralX::Teamserver.Listeners.push_back( item );
}

void CatedralNamespace::UserInterface::Widgets::ListenersTable::setDBManager( CatedralSpace::DBManager* dbManager )
{
...
void UserInterface::Widgets::ListenersTable::ListenerEdit( Util::ListenerItem item ) const
{
    for ( int i = 0; i < CatedralX::Teamserver.Listeners.size(); i++ )
    {
        if ( CatedralX::Teamserver.Listeners[ i ].Name == item.Name )
        {
            CatedralX::Teamserver.Listeners[ i ].Info = item.Info;
        }
    }
}
...
void UserInterface::Widgets::ListenersTable::ListenerRemove( QString ListenerName ) const
{
    ...
    if ( ! ListenerName.isEmpty() )
    {
        ...
        if ( ! CatedralX::Teamserver.Listeners.empty() )
        {
            for ( int i = 0; i < CatedralX::Teamserver.Listeners.size(); i++ )
            {
                if ( CatedralX::Teamserver.Listeners[ i ].Name == ListenerName.toStdString() )
                {
