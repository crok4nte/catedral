...
using namespace CatedralNamespace::UserInterface::Widgets;
using namespace CatedralNamespace::Util;

void CatedralNamespace::UserInterface::Widgets::SessionTable::setupUi(QWidget *Form, QString TeamserverName)
{
    ...
    connect( SessionTableWidget, &QTableWidget::itemSelectionChanged, this, &CatedralNamespace::UserInterface::Widgets::SessionTable::updateRow );
...
void CatedralNamespace::UserInterface::Widgets::SessionTable::NewSessionItem( Util::SessionItem item ) const
{
    ...
    CatedralX::Teamserver.Sessions.push_back( item );
    ...
    for ( auto & Session : CatedralX::Teamserver.Sessions )
    {
        ...
        if ( Session.InteractedWidget == nullptr )
        {
            ...
            if ( item.PivotParent.size() > 0 ) {
                ...
                CatedralX::Teamserver.TabSession->SessionGraphWidget->GraphPivotNodeAdd( item.PivotParent, item );
            } else {
                PivotStream = "[Pivot: "+ Util::ColorText::Cyan( "Direct" ) +"]";
                CatedralX::Teamserver.TabSession->SessionGraphWidget->GraphNodeAdd( item );
            }
...
void CatedralNamespace::UserInterface::Widgets::SessionTable::updateRow()
{
    bool selected = false;
