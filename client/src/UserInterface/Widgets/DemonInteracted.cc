...
void DemonInteracted::setupUi( QWidget *Form )
{
    ...
    {
        for ( auto& i : CatedralSpace::DemonCommands::DemonCommandList )
        {
            CompleterCommands << i.CommandString;

            if ( i.CommandString == "help" )
            {
                for ( auto & j : CatedralSpace::DemonCommands::DemonCommandList )
                {
        ...
        for ( auto& Command : CatedralX::Teamserver.AddedCommands )
        {
    ...
    DemonCommands = new CatedralSpace::DemonCommands;
...
void DemonInteracted::AppendText( const QString& text )
{
    ...
    DemonCommands->Prompt = QString(
        ColorText::Comment( CurrentDateTime() + " [" + CatedralX::Teamserver.User + "] " ) +
