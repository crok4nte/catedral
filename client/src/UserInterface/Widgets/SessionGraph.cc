...
void GraphWidget::drawBackground( QPainter* painter, const QRectF& rect )
{
    ...
    auto Background = CatedralNamespace::Util::ColorText::Colors::Hex::Background;
        ...
        for ( auto & Session : CatedralX::Teamserver.Sessions )
        {
            ...
            if ( Session.Name.compare( NodeID ) == 0 )
            {
                ...
                else if ( action->text().compare( "Mark as Dead" ) == 0 )
                {
                    auto Marked = QString();
                    Marked = "Dead";
                    CatedralApplication->CatedralAppUI.MarkSessionAs( Session, Marked );
                }
                ...
                else if ( action->text().compare( "Remove" ) == 0 )
                {
                    ...
                    for ( int i = 0; i < CatedralX::Teamserver.TabSession->SessionTableWidget->SessionTableWidget->rowCount(); i++ )
                    {
                        ...
                        {
                            CatedralX::Teamserver.TabSession->SessionTableWidget->SessionTableWidget->removeRow( i );
                        }
                    }
