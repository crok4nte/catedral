...
void Payload::setupUi( QDialog* Dialog )
{
    ...
    PayloadDialog->setMinimumSize( 950, 660 );

    gridLayout_3 = new QGridLayout( PayloadDialog );
    gridLayout_3->setObjectName( QString::fromUtf8( "gridLayout_3" ) );

    // ===== Options box (left column) =====
    ...
    LabelAgentType = new QLabel( OptionsBox );
    LabelAgentType->setObjectName( QString::fromUtf8( "LabelAgentType" ) );

    ComboAgentType = new QComboBox( OptionsBox );
    ...
    gridLayout_2->addWidget( LabelAgentType, 0, 0, 1, 1 );
    gridLayout_2->addWidget( ComboAgentType, 0, 1, 1, 1 );
    gridLayout_2->addWidget( LabelListener,  1, 0, 1, 1 );
    gridLayout_2->addWidget( ComboListener,  1, 1, 1, 1 );
    gridLayout_2->addWidget( LabelArch,      2, 0, 1, 1 );
    gridLayout_2->addWidget( ComboArch,      2, 1, 1, 1 );
    gridLayout_2->addWidget( LabelFormat,    3, 0, 1, 1 );
    gridLayout_2->addWidget( ComboFormat,    3, 1, 1, 1 );
    gridLayout_2->addWidget( TreeConfig,     4, 0, 1, 2 );

    gridLayout_2->setColumnStretch( 0, 0 );
    gridLayout_2->setColumnStretch( 1, 1 );
    gridLayout_2->setRowStretch( 4, 1 );

    // ===== Build console box (right column) =====
    ...
    // ===== Generate button (bottom row, centered) =====
    ButtonGenerate = new QPushButton( PayloadDialog );
    ButtonGenerate->setObjectName( QString::fromUtf8( "ButtonGenerate" ) );

    // ===== Assemble main grid =====
    gridLayout_3->addWidget( OptionsBox,      0, 0, 1, 1 );
    gridLayout_3->addWidget( BuildConsoleBox, 0, 1, 1, 1 );
    gridLayout_3->addWidget( ButtonGenerate,  1, 0, 1, 2, Qt::AlignHCenter );
...
auto Payload::retranslateUi() -> void
{
    ...
    if ( CatedralX::Teamserver.Listeners.size() > 0 )
    {
    ...
    for ( auto& Agents : CatedralX::Teamserver.ServiceAgents )
...
auto Payload::CtxAgentPayloadChange( const QString& AgentType ) -> void
{
    if ( ! Closed )
    {
        for ( auto& Agent : CatedralX::Teamserver.ServiceAgents )
        {
...
auto Payload::DefaultConfig() -> void
{
    ...
    auto DemonConfig             = CatedralX::Teamserver.DemonConfig;
...
auto Payload::Start() -> void
{
    ...
    // removed in Dialogs-as-Tabs (embedded in TeamserverTabWidget)
    // PayloadDialog->show();
}
