...
ts-build:
	@ echo "[*] building Catedral teamserver"
	@ ./teamserver/Install.sh
	@ cd teamserver; GO111MODULE="on" go build -ldflags="-s -w -X cmd.VersionCommit=$(git rev-parse HEAD)" -o ../catedral main.go
	@ true # setcap disabled: requires sudo; run 'make setcap-server' manually after ts-build

setcap-server:
	sudo setcap 'cap_net_bind_service=+ep' catedral
